...
```{r libraries, eval=T, message=FALSE, include=TRUE, warning=FALSE}
# load libraries
load_libs <- c('tidyverse', 'ggrepel', 'remotes', 'ggpubr', 'kableExtra')
...
```{r co2, eval=T, message=FALSE, include=TRUE}
...
df_summary <- CO2 %>% group_by(Type) %>% summarise(mean = mean(uptake),
                                     median = median(uptake))

kable(df_summary, caption = 'MTCARS data set', format = 'latex', booktabs = T) #%>%
...
#### Task 4a - Using R example datasets
Airway data set
```{r airway, eval=T, message=FALSE, include=TRUE}
...
counts %>% summarise_all(.funs = list(expressed = function(x) sum(x > 0),
                                             notExpressed = function(x) sum(x == 0))) %>% 
          pivot_longer(cols = everything()) %>% 
          separate(name, c('name', 'temp'),sep = '_') %>% 
...
# call the fucntion using previously created numneric vector
vectorInfo(numval)


# create trimmed mean fucntion
...
```{r, eval=T, message=FALSE, include=FALSE}
library('remotes')
...
```{r chromosome, fig.asp=.65, fig.width=6, fig.align='center', warning=FALSE}
# install tidybiology
...
# B. 
# plot distribution of the chromose size
...
ggplot(chromosome, aes(length_mm, mi_rna)) + 
        geom_point() +
        stat_cor(label.y = 145, method = 'spearman', cor.coef.name = 'rho') + 
        geom_smooth(method = 'lm', fill = 'gray', alpha = 0.3) +
...
```{r protein, fig.asp=.65, fig.width=6, fig.align='center', warning=FALSE}
# attach the data
data("proteins")
...
protein_summary <- proteins %>% 
          ...
          summarise(across(everything(), 
                    list(mean = mean, median = median, max = max))); print(protein_summary)
